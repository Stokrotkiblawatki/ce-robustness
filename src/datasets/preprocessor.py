...
class DatasetPreprocessor:
    def __init__(
        ...
        if self.dataset.name == "give_me_some_credit":
    ...
    def __initial_transform_prep(self) -> list[pd.DataFrame]:
        ...
        if self.sampling is True and self.sample_n is not None:
            if self.sample_n > len(X):
                raise ValueError(f"Cannot sample {self.sample_n} rows from dataset with {len(X)} rows.")
            X = X.sample(n=self.sample_n, random_state=self.sample_random_state)
            y = y.loc[X.index]
        ...
        if self.one_hot:
            ...
            # Concatenate only if there were categorical columns
    ...
    def one_hot_encode(
        ...
        X_transformed = pd.DataFrame(X_transformed, columns=X_transformed_features) # add index=X.index -> ?
    ...
    def sample_data(
        ...
        """
        Sample n observations from the train or test set.

        Parameters:
            - n: number of samples to draw
            - from_train: whether to sample from the training set (True) or test set (False)
        ...
        if n > len(X):
            raise ValueError(f"Requested sample size {n} exceeds dataset size {len(X)}.")

        random_state = 42
        X_sampled = X.sample(n=n, random_state=random_state)
