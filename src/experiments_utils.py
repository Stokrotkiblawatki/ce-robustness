...
from .classifiers.utils import array_to_tensor
import torch
...
def calculate_metrics(
    ...
    # LOF
    lof = LocalOutlierFactor(n_neighbors=13, novelty=True) #TODO zmienić n_neighbors na odpowiednie k

    lof.fit(X_train)
    lof_y_cf = lof.predict([cf])  # -1 = outlier, 1 = normal

    # radius_vals = {"give_me_some_credit": 0.3797,
    #                "heloc": 0.43,
    #                "wine_quality": 0.1,
    #                "diabetes": 0.3723,
    #                "rice": 0.0935
    #                }
    # radius = radius_vals[dataset_name]
    # distances = pairwise_distances(X_train, [cf], metric='euclidean').flatten()
    # neighbors_idx = np.where(distances <= radius)[0]
    # non = len(neighbors_idx)

    distances = pairwise_distances(X_train, [cf], metric='euclidean').flatten()

    # Get indices of the 13 closest points
    neighbors_idx = np.argsort(distances)[:13]

    non = len(neighbors_idx)

    # Agreement
    neighbors_decisions = y_train[neighbors_idx]
    if len(neighbors_idx) == 0:
        agreement = None
    else:
        agreement = np.sum(neighbors_decisions == cf_label)/len(neighbors_idx)

    # Accuracy change
    acc_change = m1.accuracy-m2.accuracy
...
def perturb_and_evaluate(models, dataset_name, X_test, y_test):
    delta_dict = {"rice": 0.15, "diabetes": 0.075, "heloc": 0.075, "wine_quality": 0.075, "give_me_some_credit": 0.35, "breast_cancer": 0.1, "adult": 0.1, "car_eval": 0.15}
    for mlp in models:
        acc_0 = eval_fn(mlp, X_test, y_test)
        acc_change = 0
        i = 0
        while np.abs(acc_change) < 0.005 or np.abs(acc_change) > 0.1:
            delta = delta_dict[dataset_name]

            mlp_copy = deepcopy(mlp)

            # Apply perturbations to the copy
            with torch.no_grad():
                for layer in mlp_copy.layers:
                    if isinstance(layer, torch.nn.Linear):
                        perturbation = torch.empty_like(layer.weight).uniform_(-delta, delta)
                        layer.weight.data += perturbation

            acc_1 = eval_fn(mlp_copy, X_test, y_test)
            acc_change = acc_0 - acc_1

            # If acceptable, apply the perturbations to original model
            if 0.005 <= np.abs(acc_change) <= 0.1:
                with torch.no_grad():
                    for orig_layer, copy_layer in zip(mlp.layers, mlp_copy.layers):
                        if isinstance(orig_layer, torch.nn.Linear):
                            orig_layer.weight.data.copy_(copy_layer.weight.data)

            i += 1

        accuracy = eval_fn(mlp, X_test, y_test)
        print("Old accuracy is", mlp.accuracy)

        mlp.accuracy = accuracy
        print("New accuracy is", mlp.accuracy)

    return models

def eval_fn(mlp, X_test, y_test):
    X_test = array_to_tensor(X_test, device="cpu")
    y_test = array_to_tensor(y_test, device="cpu")
    y_pred = mlp.predict_crisp(X_test)
