...
class MLPClassifier(nn.Module):
    ...
    def __getstate__(self):
        # Save all attributes in __dict__, including custom ones like accuracy
        return self.__dict__

    def __setstate__(self, state):
        # Restore all attributes when loading
...
) -> tuple[MLPClassifier, callable, callable]:
    """
...
def train_K_mlps(
    ...
    print(
        f"Ensemble: Average accuracy: {np.mean(accuracies)}, Average recall: {np.mean(recalls)}, Average precision: {np.mean(precisions)}, Average f1: {np.mean(f1s)}"
    )
    print(
        f"Ensemble: Std accuracy: {np.std(accuracies)}, Std recall: {np.std(recalls)}, Std precision: {np.std(precisions)}, Std f1: {np.std(f1s)}"
    )
    return {
        "models": models,
...
def ensemble_predict_proba(
    models: list[MLPClassifier], X: Union[np.ndarray, torch.Tensor]
) -> np.ndarray[float]:
    """
...
def ensemble_predict_crisp(
    sample: np.ndarray, models: list[MLPClassifier], class_threshold: float = 0.5
) -> np.ndarray[int]:
    """
