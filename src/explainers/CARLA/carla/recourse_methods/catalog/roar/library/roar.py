...
def roar_recourse(
    ...
    if loss_type == "MSE":
        ...
        with torch.no_grad():
            pred = torch_model.predict_crisp(x)
        y_target = [0, 1]
        y_target = torch.tensor(y_target).float().to(device)
        if pred == 0:
            target_class = torch.round(y_target[1]).float()
        else:
            target_class = torch.round(y_target[0]).float()
        #print("target class in roar is", target_class)
        #print("pred for the x is", pred)
        loss_fn = torch.nn.BCELoss()
    else:
    ...
    while loss_diff > loss_threshold:
        ...
        #print(f"Step {i}, pred logit: {f_x_new.item():.4f}, loss: {loss.item():.4f}, target: {target_class.item():.4f}")
        #i += 1
