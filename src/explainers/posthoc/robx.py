# Dutta's work https://arxiv.org/pdf/2207.02739
...
def counterfactual_stability(
    ...
    ), "variance must be a numpy array or a float"
...
def robx_algorithm(
    ...
    if counterfactual_stability_test(
        counterfactual_stability(
            start_counterfactual, predict_class_proba_fn, variance, N
        ),
        tau,
    ):
    ...
    for iteration in range(robx_max_iter):
        for i, (cf, cf_conservative) in enumerate(
            zip(counterfactuals, conservative_counterfactuals)
        ):
            cf = (
                robx_lambda * cf_conservative + (1 - robx_lambda) * cf
            )  # convex combination
            history[i].append(cf)
            counterfactuals[i] = cf

            if counterfactual_stability_test(
                counterfactual_stability(cf, predict_class_proba_fn, variance, N), tau
            ):
...
class RobX(PostHocExplainer):
    def __init__(
        self, X_train: np.ndarray | pd.DataFrame, predict_proba_fn: object
    ) -> None:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if isinstance(start_instance, pd.DataFrame):
            start_instance = start_instance.to_numpy()
