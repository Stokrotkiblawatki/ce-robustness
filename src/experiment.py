...
class Experiment:
    ...
    def run(self) -> None:
        ...
        for combination in all_combinations:
            ...
            preprocessor = DatasetPreprocessor(
                ...
                one_hot=True,
                sampling=self.cfg_exp["sampling"],
                sample_n=self.cfg_exp["sample_n"],
                sample_random_state=self.cfg_exp["sample_random_state"],
            )
            ...
            if exp_type == "Weights":
                m = models_m1[0]
                models_m2 = [deepcopy(m) for _ in range(self.cfg_exp["m2_count"])]

                print(eval_fn(models_m2[0], X_test, y_test))

                models_m2 = perturb_and_evaluate(models_m2, dataset_name, X_test, y_test)
            ...
            new_row = pd.DataFrame({
                ...
                "exp_type": [exp_type],
                "explainer": [explainer],
                "acc_m1": [models_m1[0].accuracy],
                "acc_m2": [[m.accuracy for m in models_m2]]
            })

            # Saving accuracy change magnitude
            csv_file = "acc_change.csv"

            new_row.to_csv(csv_file, mode='a', header=not pd.io.common.file_exists(csv_file), index=False)
            ...
            for base_hp, base_hp_to_save in e2e_combs:
                ...
                for _i, idx in enumerate(indices):
                    for m2 in models_m2:
                        ...
                        t0 = time.time()
                        try:
                            if explainer_name == 'rbr': #or 'roar':
                                query_instance = x.copy()
                                query_instance["__y_test__"] = y
                                b_cf = base_explainer.generate(query_instance)
                            else:
                        ...
                        # To increment the progress bar up to 100%
                        self.global_iter += 1
                        self.pbar.update(1)
            self.save_results()
        self.pbar.close()
    ...
    def robust_part(
        ...
        for ph_comb in ph_combs:
            ...
            if not check_is_none(rb_cf):
                ...
                # Distance from base counterfactual
                ...
                logging.warning(f"Skipping {ph_explainer_name} with combination {ph_comb} because rb_cf is None")
    ...
    def get_m1_m2_records(
        ...
        record = {
            ...
            f"The same decision for both models": m1_pred_crisp == m2_pred_crisp,
        ...
        mets = {
            ...
            f"{cf_prefix}counterfactual_lof": metrics_m1["lof"],
            f"{cf_prefix}counterfactual_num_of_neigh": metrics_m1["non"],
            f"{cf_prefix}counterfactual_agreement": metrics_m1["agreement"],
            f"{cf_prefix}counterfactual_acc_change": metrics_m1["acc_change"],
    ...
    def sample_classifiers(
        ...
        np.random.seed(self.cfg_gen["random_seed"])
        models = []
    ...
    def _setup(self) -> None:
        ...
        # self.cfg_gen["model_path"] = os.path.join(self.cfg_gen["model_path"]) # czy powinnam to odkomentować?
